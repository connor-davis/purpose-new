...
import AddTrainingModal from "../../components/modals/training/add";
import EditTrainingModal from "../../components/modals/training/edit";
...
const TrainingPage = () => {
  ...
  const [editingTraining, setEditingTraining] = createSignal(undefined);
  ...
  const fetchTraining = () => {
    setTimeout(async () => {
      ...
      const { data, totalTraining, totalPages } = response.data;

      if (data.length === 0 && totalTraining > 0) {
  ...
  const deleteTraining = async (id) => {
    const response = await axios.delete(apiUrl + "training/" + id, {
      headers: { Authorization: "Bearer " + user.token },
    });
            ...
            <table class="table-auto bg-white border-l border-t border-r border-neutral-200 text-black">
              <thead>
                <tr class="p-3 border-b border-neutral-200 bg-neutral-900 text-white select-none">
                  ...
                  <th class="text-left p-3">Number Trained</th>
                ...
                <For each={training()}>
                  {(training) => (
                    <tr class="hover:bg-neutral-100 border-b border-neutral-200 group">
                      <td class="p-3 whitespace-nowrap">{training.date}</td>
                      <td class="p-3 whitespace-nowrap">
                        {training.numberTrained}
                      </td>
                      <td class="p-3 whitespace-nowrap">
                      ...
                      <td class="p-3">
                        <div class="flex w-24 h-full items-center space-x-2">
                          ...
                          <div
                            onClick={() => setShowDelete(training)}
