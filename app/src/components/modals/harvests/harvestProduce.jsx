...
const HarvestProduceModal = ({
  ...
  const [produce, setProduce] = createSignal([]);
  ...
  const updateHarvestProduce = async () => {
    const _produce = produce()
      .map((produceItem) => {
        const produceYield = metadata.harvestProduce[produceItem._id]
          ? metadata.harvestProduce[produceItem._id].yield
          : undefined;
        const produceWeight = metadata.harvestProduce[produceItem._id]
          ? metadata.harvestProduce[produceItem._id].weight
          : undefined;

        return {
          _id: produceItem._id,
          yield: produceYield,
          weight: produceWeight,
        };
      })
      .filter((produceItem) => produceItem !== undefined);

    added(_produce, metadata);
  };
                ...
                <table class="table-auto bg-white border-l border-t border-r border-neutral-200 text-black">
                  <thead>
                    <tr class="p-3 border-b border-neutral-200 bg-neutral-900 text-white select-none">
                      ...
                      <th class="p-3 w-24">Yield (kg)</th>
                      <th class="p-3 w-24">Weight (kg)</th>
                    </tr>
                  </thead>
                  <tbody>
                    <For each={produce()}>
                        ...
                        <tr class="hover:bg-neutral-100 border-b border-neutral-200 group">
                          ...
                          <td class="p-3">
                            <input
                              ...
                              value={
                                metadata.harvestProduce
                                  ? metadata.harvestProduce[produceItem._id]
                                    ? metadata.harvestProduce[produceItem._id]
                                        .yield
                                    : ""
                                  : ""
                              ...
                              onChange={(event) => {
                                let newHarvestProduce = {
                                  ...metadata.harvestProduce,
                                };

                                newHarvestProduce[produceItem._id] = {
                                  ...(newHarvestProduce[produceItem._id] || {}),
                                  yield: event.target.value,
                                };

                                setMetadata({
                                  harvestProduce: newHarvestProduce,
                                });
                              }}
                              type="number"
                              placeholder="Yield"
