...
const AddWasteModal = ({ added = () => {}, closed = () => {} }) => {
  ...
  const [wasteDate, setWasteDate] = createSignal(undefined);
  const [wasteKgs, setWasteKgs] = createSignal(undefined);
  const [wasteType, setWasteType] = createSignal(undefined);
  ...
  const addWaste = async () => {
    ...
    const response = await axios.post(apiUrl + "waste", data, {
      headers: { Authorization: "Bearer " + user.token },
    });

    if (response.data.error) return setErrorMessage(response.data.message);
    else {
      setSuccessMessage("Successfully added waste.");
    ...
    <div class="absolute bg-neutral-900 bg-opacity-50 left-0 top-0 right-0 bottom-0 flex flex-col items-center justify-center animate-fade-in">
      <div class="flex flex-col w-full md:w-3/5 h-full overflow-y-auto  space-y-3 bg-neutral-900 border-l border-t border-r border-b border-neutral-700 rounded p-2">
        <div class="flex items-center justify-between w-full">
          <div class="cookie text-white text-2xl">Add Waste</div>
          <div
            onClick={() => closed()}
        ...
        >
          <div class="flex flex-col w-full space-y-1">
            ...
            <DatePicker onChange={(date) => setWasteDate(date)} />
